...
i = 0
name=""
filename="./highknees/"+filename1+".csv"
        ...
        try:
            ...
            if not is_pushup(results,height,width,th=4)and isin(results,0.4)==100 and angle_lk>110 and\
                angle_rk>110 and angle_lh>100 and angle_rh>100 and knee_position(results)and is_standing(results):
                 state="U"
                 name="standing"  
            elif not is_pushup(results,height,width,th=4) and isin(results,0.5)==100 and angle_rk>100 and angle_rh>120 and angle_lh<120\
                  and angle_lk<100 and knee_ankle_distance(results)\
                      and not knee_position(results) and is_standing(results):
                    state="D"
                    name="left knee high"
                    list_state.append(name)
            elif not is_pushup(results,height,width,th=4)and isin(results,0.5)==100  and angle_lk>100 and angle_rh<120 and angle_lh>120 \
                  and angle_rk<100 and knee_ankle_distance(results)\
                      and not knee_position(results) and is_standing(results):
                  state="E"
                  name="right knee high"
            else:
                name="None"
            row.append(name)
            writer.writerow(row)
